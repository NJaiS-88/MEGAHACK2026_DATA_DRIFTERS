...
if not gemini_api_key:
    ...
    try:
        if USE_NEW_API:
            ...
            model = "gemini-1.5-flash"  # Use confirmed working model
...
Analyze if the student's answer is semantically correct, partially correct, or incorrect. 
Even if the wording is different, if the core concept is correct, mark it as "correct".
If they are on the right track but missing key details, mark it as "partial".
If they are fundamentally wrong, mark it as "incorrect".

Respond ONLY with a valid JSON object with these exact keys:
- "evaluation_state": "correct" | "partial" | "incorrect"
- "misconception": boolean (true if there are flaws/misconceptions)
- "confidence": number between 0 and 1
- "feedback": string (helpful feedback message, 2-3 sentences)
"""
    ...
    try:
        ...
        return {
            "evaluation_state": str(result.get("evaluation_state", "incorrect")),
