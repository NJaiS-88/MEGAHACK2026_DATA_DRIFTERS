...
function QuizPanel({ concept, onClose }) {
  ...
  return (
    <div className="quiz-panel" style={{
      width: '450px', // Increased width for better history view
      height: '100%',
      ...
      <div style={{
        ...
        <div>
          ...
          <span style={{ fontSize: '0.8rem', color: '#9ca3af' }}>Mastery & Practice</span>
        </div>
        <button 
          onClick={onClose}
          style={{ 
      ...
      <div style={{ padding: '1.5rem', overflowY: 'auto', flex: 1 }}>
        ...
        {questions.length > 0 && (
          <div style={{ marginBottom: '2rem' }}>
            <h3 style={{ fontSize: '1rem', color: '#38bdf8', marginBottom: '1rem', textTransform: 'uppercase', letterSpacing: '0.05em' }}>Current Quiz</h3>
            {questions.map((q, idx) => (
              <QuestionCard 
                key={idx} 
                index={idx} 
        ...
        <div style={{ marginTop: '3rem' }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '1rem' }}>
            ...
            <span style={{ fontSize: '0.75rem', color: '#9ca3af' }}>{attempts.length} total</span>
          </div>
            ...
            {['all', 'correct', 'incorrect', 'misconceptions'].map(f => (
              <button
                ...
                style={{
                  ...
                  borderRadius: '9999px',
                  fontSize: '0.75rem',
                  backgroundColor: filter === f ? '#38bdf8' : '#1e293b',
                  color: filter === f ? '#020617' : '#9ca3af',
                  border: 'none',
                  cursor: 'pointer',
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
            {filteredAttempts.length === 0 ? (
              <p style={{ color: '#4b5563', textAlign: 'center', fontSize: '0.85rem' }}>No attempts found for this filter.</p>
            ) : (
              filteredAttempts.map((attempt, i) => (
                <div key={i} style={{
                  ...
                  {attempt.misconception && attempt.misconception.misconception_detected && (
                    <div style={{ marginTop: '0.75rem', padding: '0.5rem', backgroundColor: '#450a0a40', borderRadius: '4px', border: '1px solid #450a0a' }}>
